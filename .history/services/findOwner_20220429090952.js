...
async function findOwner(req, res) {
  pool.query("SELECT address FROM smartcontracts", (error, result) => {
    ...
    ReadIdToken(contracts[0].address);
...
async function ReadOwner(a) {
  console.log("Count", countContracts);
  var config = {
    method: "get",
    url: `${baseURL}?contractAddress=${contracts[0].address}&tokenId=1`,
    headers: {},
  };
  axios(config)
    .then((response) => { return JSON.stringify(response.data.owners[0], null, 2)}
    )
