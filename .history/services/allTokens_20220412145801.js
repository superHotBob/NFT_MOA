...
const readSmartContractsAddress = async (req, res) => {
  pool.query("SELECT address FROM smartcontracts", (error, result) => {
    ...
    callGetNFTsForCollectionOnce(res,req, startToken = "");
  });
};
...
let totalNftsFound = [];
let m = 0;
async function callGetNFTsForCollectionOnce(res,req, startToken = "") {
  const url = `${baseURLOne}/?contractAddress=${contracts[0].address}&startToken=${startToken}`;
  ...
  if (nextToken) {
    callGetNFTsForCollectionOnce(contracts, nextToken);
  } else {
    m = totalNftsFound.flat().length;
    console.log(m);
    res.send(
      `<h2 style="margin-top: 20vh;text-align: center;">
      For contract ${contracts[0]}  <br/> 
      ${m} tokens
      </h2>`
    )
    //  writeToBase(a)
