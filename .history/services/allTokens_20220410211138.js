...
async function callGetNFTsForCollectionOnce(a, startToken = "") {
  ...
  console.log(response.data.nfts.map((i) => i.id.tokenId).length);
  let n = response.data.nfts.map((i) => i.id.tokenId).length;

  function writeToBase() {
    if (n > -1) {
      const tokenId = response.data.nfts[n].id.tokenId;
      const config = {
        method: "get",
